...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-[#020617] font-sans text-slate-100 overflow-hidden relative">
      {/* Background Decorative Ethereal Blobs */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div 
          ...
          className="absolute -top-[20%] -left-[10%] w-[70%] h-[70%] bg-indigo-600/10 rounded-full blur-[140px]"
        />
        <motion.div 
          animate={{ 
            scale: [1, 1.4, 1],
          ...
          className="absolute -bottom-[25%] -right-[15%] w-[60%] h-[60%] bg-violet-600/10 rounded-full blur-[120px]"
        />
        <motion.div 
          animate={{ 
          ...
          className="absolute top-[20%] right-[30%] w-[40%] h-[40%] bg-cyan-500/5 rounded-full blur-[100px]"
        />
      </div>
      ...
      <LayoutGroup>
        ...
        <motion.aside 
          ...
          className="hidden md:flex w-85 flex-col glass-panel z-50 rounded-r-[2.5rem] border-r-indigo-500/10"
        >
        ...
        <main className="flex-1 flex flex-col relative max-w-full">
          {/* Header */}
          <header className="h-28 flex items-center justify-between px-12 bg-[#020617]/50 backdrop-blur-3xl border-b border-white/[0.03] sticky top-0 z-40">
            <div className="flex items-center gap-4 md:hidden">
               <div className="bg-gradient-to-tr from-indigo-600 to-violet-600 p-2.5 rounded-2xl shadow-xl shadow-indigo-600/20"><Zap className="text-white w-6 h-6 fill-white" /></div>
               <span className="font-black text-3xl tracking-tighter text-white">Bework</span>
            </div>
            <div className="hidden md:flex items-center gap-10">
               <div className="flex items-center gap-3.5 px-5 py-2.5 bg-indigo-500/5 rounded-2xl border border-indigo-500/20">
                 <motion.div 
                    animate={{ opacity: [0.3, 1, 0.3], scale: [1, 1.2, 1] }} 
                    transition={{ repeat: Infinity, duration: 1.5 }}
                    className="w-2.5 h-2.5 bg-green-400 rounded-full shadow-[0_0_15px_rgba(74,222,128,0.8)]" 
                 />
                 <span className="text-[12px] font-black text-indigo-300 uppercase tracking-[0.25em]">Neural Sync Engine v11.4</span>
               </div>
               <div className="flex items-center gap-8 text-[11px] font-black text-slate-500 uppercase tracking-[0.25em]">
                 <span className="flex items-center gap-2.5 cursor-pointer hover:text-white transition-all group">
                   <LayoutDashboard className="w-4.5 h-4.5 group-hover:text-indigo-400 group-hover:-translate-y-0.5 transition-transform" /> Dashboard
                 </span>
                 <span className="flex items-center gap-2.5 cursor-pointer hover:text-white transition-all group">
                   <Shield className="w-4.5 h-4.5 group-hover:text-indigo-400 group-hover:-translate-y-0.5 transition-transform" /> Governance
                 </span>
                 <span className="flex items-center gap-2.5 cursor-pointer hover:text-white transition-all group">
                   <Terminal className="w-4.5 h-4.5 group-hover:text-indigo-400 group-hover:-translate-y-0.5 transition-transform" /> Protocol
                 </span>
               </div>
            </div>
            <div className="flex items-center gap-5">
              <motion.div 
                 ...
                 className="p-3.5 rounded-2xl bg-white/5 border border-white/5 cursor-pointer text-slate-400 hover:text-white transition-all"
              >
                <Search className="w-6 h-6" />
              </motion.div>
              <button className="px-8 py-4 bg-gradient-to-r from-indigo-600 via-indigo-500 to-indigo-600 bg-[length:200%_auto] hover:bg-right text-white rounded-[1.5rem] text-[12px] font-black uppercase tracking-[0.2em] transition-all shadow-[0_20px_50px_rgba(79,70,229,0.25)] hover:scale-105 active:scale-95">
                 Neural Node v1.0
              </button>
            </div>
          </header>
          ...
          {/* Chat Area */}
          <div className="flex-1 overflow-y-auto px-8 py-20 md:px-24 lg:px-44 scrollbar-hide">
            <AnimatePresence mode="wait">
              {!currentSession || (messages.length === 0 && !isLoading) ? (
                ...
                >
                  <motion.div variants={item} className="relative mb-16">
                    <motion.div 
                      animate={{ rotate: -360 }}
                      transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
                    ...
                    <motion.div 
                      ...
                      className="relative bg-white p-12 rounded-[3.5rem] shadow-[0_40px_100px_rgba(99,102,241,0.3)] border border-white"
                    >
                      <Zap className="w-28 h-28 text-indigo-600 fill-indigo-600 stroke-[1.5px]" />
                    </motion.div>
                  </motion.div>

                  <motion.h1 
                    variants={item}
                    className="text-6xl md:text-8xl font-black text-white mb-10 tracking-tighter leading-[0.95]"
                  >
                  ...
                  <motion.p 
                    variants={item}
                    className="text-slate-400 mb-20 text-2xl font-semibold leading-relaxed max-w-2xl mx-auto opacity-70"
                  >
                    Bework is your high-bandwidth neural link for strategic growth, operational excellence, and technical mastery.
                  </motion.p>
                  
                  <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-3xl px-6">
                    {QUICK_SUGGESTIONS.map((suggestion, idx) => (
                      <motion.button
                        ...
                        className="group flex items-center justify-between px-10 py-8 bg-white/[0.04] border border-white/5 rounded-[2.75rem] text-left transition-all duration-500 backdrop-blur-3xl shadow-2xl shadow-indigo-500/5"
                      >
                        <span className="text-sm font-black text-slate-200 tracking-tight leading-tight">{suggestion}</span>
                        <div className="p-4 bg-slate-900 rounded-2xl group-hover:scale-110 group-hover:bg-indigo-600 transition-all duration-500">
                          <Sparkles className="w-5 h-5 text-indigo-400 group-hover:text-white" />
                        </div>
                      </motion.button>
                    ))}
                  </motion.div>
          ...
          {/* Input Area */}
          <div className="p-10 md:p-18 bg-gradient-to-t from-[#020617] via-[#020617]/90 to-transparent sticky bottom-0 w-full z-40">
            <div className="max-w-5xl mx-auto relative">
              ...
              >
                {/* Neon glow effect */}
                <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 via-violet-500 to-indigo-500 rounded-[3.5rem] blur-3xl opacity-10 group-focus-within:opacity-50 transition-all duration-700 animate-pulse"></div>
                
                <div className="relative flex items-center bg-[#070d1d] border-2 border-white/5 rounded-[3rem] shadow-[0_40px_100px_rgba(0,0,0,0.6)] group-focus-within:border-indigo-500/40 transition-all backdrop-blur-4xl">
                  <input
                    type="text"
                    value={input}
                    onChange={(e) => setInput(e.target.value)}
                    placeholder="Brief Bework AI on your next directive..."
                    className="w-full pl-12 pr-32 py-9 bg-transparent focus:outline-none text-white font-black text-xl placeholder:text-slate-700 tracking-tight"
                  />
                  <div className="absolute right-5 flex items-center gap-3">
                    <button
                      disabled={isLoading || !input.trim()}
                      className="group flex items-center justify-center gap-3 h-16 px-10 bg-white hover:bg-white text-slate-900 hover:text-indigo-600 disabled:bg-slate-800 disabled:text-slate-600 rounded-[2rem] transition-all shadow-3xl active:scale-95 disabled:scale-100 font-black text-sm uppercase tracking-[0.2em] relative overflow-hidden"
                    >
                      <motion.div 
                        initial={false}
                      ...
                      >
                        {isLoading ? <Loader2 className="w-6 h-6 animate-spin" /> : <Send className="w-6 h-6 mr-1 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />}
                        {isLoading ? '' : 'Transmit'}
                      </motion.div>
                    </button>
                  </div>
              ...
              <div className="flex items-center justify-between px-12 mt-8">
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em]">Strategic Protocol</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-violet-500 rounded-full"></div>
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em]">End-Locked Security</span>
                  </div>
                </div>
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] opacity-40">Build 0x92f - persistent intelligence</span>
              </div>
            </div>
          </div>
        </main>
